...
class initiate_robot(object):
    ...
    def produce_scoop_dmp(self,bowl_pos_x, bowl_pos_y, bowl_pos_z):
        ...
        # file_name = 'scoop_dmp_pose_test.csv'
        ...
        for row in range(y_track.shape[0]):
            ...
            #writer.writerow(normalised_final)
    ...
    def get_ik_traj(self, scooping_traj):
        gik = GetIK('xarm6')
        save_path = '/home/chi-onn/fyp_ws/src/scooping/traj_files'
        file_name = 'scoop_dmp_ik.csv'
        completeName = os.path.join(save_path,file_name)
        f = open(completeName, 'w')
        writer = csv.writer(f)
        for row in range(scooping_traj.shape[0]):
            ps = PoseStamped()
            ps.pose.position.x = scooping_traj.iloc[row,0]
            ps.pose.position.y = scooping_traj.iloc[row,1]
            ps.pose.position.z = scooping_traj.iloc[row,2]
            ps.pose.orientation.x = scooping_traj.iloc[row,3]
            ps.pose.orientation.y = scooping_traj.iloc[row,4]
            ps.pose.orientation.z = scooping_traj.iloc[row,5]
            ps.pose.orientation.w = scooping_traj.iloc[row,6]
            resp = gik.get_ik(ps)
...
def main():
    try:
        ...
        myrobot.get_ik_traj(scoop_traj)
